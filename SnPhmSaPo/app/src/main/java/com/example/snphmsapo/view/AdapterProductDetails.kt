...
class AdapterProductDetails(
    var listVariantDetail: MutableList<Variant>
) : RecyclerView.Adapter<AdapterProductDetails.ItemProductDetailViewHolder>() {

    var onClickItemVariant: ((idProduct: Int, idVariant: Int) -> Unit)? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemProductDetailViewHolder {
        val binding =
            ItemVariantDetailBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ItemProductDetailViewHolder(binding)
    }

    override fun onBindViewHolder(holder: ItemProductDetailViewHolder, position: Int) {
    ...
    inner class ItemProductDetailViewHolder(var binding: ItemVariantDetailBinding) :
        ...
        fun bind(item: Variant) {
            ...
            binding.txtVariantDetailAvailable.text =
                item.inventories?.let { Variant.totalAvailable(it) }?.formatNumber()
            binding.txtVariantDetailOnHand.text =
                item.inventories?.let { Variant.totalOnHand(it) }?.formatNumber()
